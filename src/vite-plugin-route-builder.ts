        ...
        if (route.lazy && metadata?.fs) {
          ...
          // Strategy 7: For special path correction
            ...
            if (fsPath.endsWith('/')) {
              ...
              if (
                isSync &&
                fileToImportMap[`${correctedPath}/index.sync.tsx`]
              ) {
                ...
                isSync &&
                fileToImportMap[`${correctedPath}.sync.tsx`]
              ) {
          ...
          if (matchedKey && fileToImportMap[matchedKey]) {
            ...
            logger.warn(
              `[route-builder-v2] Could not find file for fs path: ${fsPath}`,
            )
            logger.warn(
      ...
      return routes.map((route) => {
        ...
        if (route.lazy && metadata?.fs) {
          ...
          if (matchedKey) {
            ...
            delete newRoute.lazy
            logger.warn(
              `[route-builder-v2] No matching file for route: ${fsPath}`,
            )
          }
        }
...
`
  ...
  function generateRoutes() {
    try {
      ...
      logger.error(
        `[route-builder-v2] Error generating routes:${error.message}`,
      )
      console.error(error)
      throw error
    }
